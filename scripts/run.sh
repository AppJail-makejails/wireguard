...
main()
{
    ...
    case "${cmd}" in
        add|check|del|get-addr|get-network-addr|init|show|showqr) ${cmd} "$@" ;;
        *) usage; exit ${EX_USAGE} ;;
    esac
}
...
EOF
    ...
    if [ -f "${WG_DNS_FILE}" ]; then
        local dns
        dns=`head -1 -- "${WG_DNS_FILE}"` || exit $?

        echo "DNS = ${dns}" >> "${peerdir}/wg.conf" || exit $?
    fi

    local allowedips

    if [ -f "${WG_ALLOWEDIPS_FILE}" ]; then
        allowedips=`head -1 -- "${WG_ALLOWEDIPS_FILE}"` || exit $?
    else
        allowedips="${network}"
    fi
...
init()
{
    if [ ! -f /.init ]; then
        info "Initializing ..."
        /scripts/init.sh || exit $?

        info "Configuring IP forwarding"
        sysrc gateway_enable=YES || exit $?
    ...
    if ! service wireguard status > /dev/null 2>&1; then
...
showqr()
{
    ...
    show "${ident}" | qrencode -t ansiutf8
}
...
{
    cat << EOF
usage: wg-xcaler add <ident>
       wg-xcaler check <ident>
       wg-xcaler del <ident>
       wg-xcaler get-addr <ident>
       wg-xcaler get-network-addr
       wg-xcaler init
       wg-xcaler show <ident>
       wg-xcaler showqr <ident>
EOF
}
